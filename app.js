...
/* '전체메뉴 펼치기' 상태 설정 + 시연·기능 버튼 활성 표시 */
function setExpandAll(on){
  s1state.menuExpandAll = on;
  const btn = document.querySelector('#sianList [data-expandall]');
...
function s1nav(patch){
  setExpandAll(false);             // 화면 이동 시 '전체메뉴 펼치기' 상태 해제
...
document.addEventListener('click', (e)=>{
  ...
  if(t.closest('[data-expandall]')){
    const open = document.getElementById('menuDrawer');
    if(open && s1state.menuExpandAll){ setExpandAll(false); closeMenuDrawer(); }
    else { setExpandAll(true); s1state.amCat='self'; openMenuDrawer(); }
    return;
  }
  ...
  if(t.closest('[data-menuclose]') || (t.classList && t.classList.contains('menu-ov'))){ setExpandAll(false); closeMenuDrawer(); return; }
